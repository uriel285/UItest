/* // /src/app/api/backblaze/upload.ts
...
const upload = multer({
  ...
  fileFilter: (req: any, file: { mimetype: string; }, cb: (arg0: Error | null, arg1: boolean | undefined) => void) => {
    // Puedes agregar un filtro para los tipos de archivos si es necesario
...
// Handler de la API
export default async function handler(req: any, res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: { error?: string; message?: string; file?: string; }): void | PromiseLike<void>; new(): any; }; }; }) {
  if (req.method === 'POST') {
    // Usamos Multer como middleware
    upload.single('file')(req, res, async (err: { message: any; }) => {
      ...
      try {
        ...
        // Subir el archivo a Backblaze B2 (S3 compatible)
        const params = {
          Bucket: bucketName,
        ...
        console.log("Subiendo archivo a Backblaze...");
        ...
        res.status(200).json({ message: 'Archivo cargado exitosamente', file: data.Location });
      } catch (error) {
        console.error(error);
